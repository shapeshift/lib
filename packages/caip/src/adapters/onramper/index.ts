...
const AssetIdToOnRamperTickerMap = {
  [btcAssetId]: 'btc',
  [cosmosAssetId]: 'atom',
  [ethAssetId]: 'eth',
...
const onRamperTickerToAssetIdMap = invert(AssetIdToOnRamperTickerMap)

export const getSupportedOnRamperAssets = () =>
  entries(AssetIdToOnRamperTickerMap).map(([assetId, ticker]) => ({
    assetId,
    ticker,
  }))

export const onRamperTickerToAssetId = (id: string): string | undefined => onRamperTickerToAssetIdMap[id.toLowerCase()]