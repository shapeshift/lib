...
export class FoxyApi {
  ...
  async getTimeUntilClaimable(input: TxInputWithoutAmountAndWallet): Promise<string> {
    ...
    const blocksLeftInFutureEpochs = epochsLeft.minus(1).gt(0) ? epochsLeft.minus(1).times(epoch.length) : 0 // don't count current epoch
  ...
  // estimated apy
  apy(): string {
    return process.env.FOXY_INVESTOR_APY ?? '.2'
  }
