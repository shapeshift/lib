  ...
  describe('getMarketCap', () => {
    ...
    it('can map coincap to AssetIds', async () => {
      mockedAxios.get
        ...
        .mockResolvedValue({ data: { data: [btc] } })
      const result = await coinMarketService.findAll()
      const bitcoinAssetIds = adapters.coincapToAssetId('bitcoin')
      const etherAssetIds = adapters.coincapToAssetId('ethereum')
      const [btcKey, ethKey] = Object.keys(result)
      expect(btcKey).toEqual(bitcoinAssetIds)
      expect(ethKey).toEqual(etherAssetIds)
    })
  })
