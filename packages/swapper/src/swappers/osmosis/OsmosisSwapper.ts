...
export class OsmosisSwapper implements Swapper<ChainId> {
  ...
  async buildTrade(args: BuildTradeInput): Promise<Trade<ChainId>> {
    const { sellAsset, sellAssetBip44Params, buyAsset, sellAmount, receiveAddress } = args
  ...
  async getTradeQuote(input: GetTradeQuoteInput): Promise<TradeQuote<ChainId>> {
    const { sellAsset, buyAsset, sellAmount, sellAssetBip44Params } = input
    if (!sellAmount) {
      throw new SwapError('sellAmount is required', {
        code: SwapErrorTypes.RESPONSE_ERROR,
      })
  ...
  async executeTrade({ trade, wallet }: ExecuteTradeInput<ChainId>): Promise<OsmosisTradeResult> {
    const { sellAsset, buyAsset, sellAmount, sellAssetBip44Params, receiveAddress } = trade
    ...
    if (!isFromOsmo) {
      sellAddress = await cosmosAdapter.getAddress({ wallet, bip44Params: sellAssetBip44Params })
      ...
      sellAddress = await osmosisAdapter.getAddress({ wallet, bip44Params: sellAssetBip44Params })
