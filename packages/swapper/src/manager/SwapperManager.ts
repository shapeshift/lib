...
function validateSwapper(swapper: Swapper<ChainNamespace>) {
...
export class SwapperManager {
  public swappers: Map<SwapperType, Swapper<ChainNamespace>>

  constructor() {
    this.swappers = new Map<SwapperType, Swapper<ChainNamespace>>()
  }
   ...
   */
  addSwapper(swapperType: SwapperType, swapperInstance: Swapper<ChainNamespace>): this {
   ...
   */
  getSwapper(swapperType: SwapperType): Swapper<ChainNamespace> {
    const swapper = this.swappers.get(swapperType)
    if (!swapper)
  ...
  async getBestSwapper(args: ByPairInput): Promise<Swapper<ChainNamespace> | undefined> {
   ...
   * @returns {SwapperType}
   */
  getSwappersByPair(pair: ByPairInput): Swapper<ChainNamespace>[] {
    const { sellAssetId, buyAssetId } = pair
    return Array.from(this.swappers.values()).filter(
      (swapper: Swapper<ChainNamespace>) =>
  ...
  getSupportedBuyAssetIdsFromSellId(args: BuyAssetBySellIdInput) {
    return uniq(
      Array.from(this.swappers.values()).flatMap((swapper: Swapper<ChainNamespace>) =>
