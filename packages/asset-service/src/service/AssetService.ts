...
const getRenderedIdenticonB64 = (symbol: string, tokenId: string): string => {
  ...
  const offsetX = Math.round((imageSize - pixelSymbolWidth * symbolScale) / 2)
  const offsetY = Math.round((imageSize - pixelSymbolHeight * symbolScale) / 2)
